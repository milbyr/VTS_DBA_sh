...
function FN_IS_WEB_TIER {
  if [ $(hostname) = "${L_IASSRV}" ]; then
    L_RES=0
  else
    L_RES=1
  fi
  exit $L_RES
} ### End of FN_IS_WEB_TIER ###
...
function FN_Get_Edition
{
  ...
  if  [ -f ${L_ED_FN} ]; then
    ...
    export L_APPS_RUN_FS=$(awk '/fs/ {print $3}' ${L_ED_FN} | head -1 )
...
function FN_Create_Pairsfile
{
  ...
  L_DOMAIN=`grep $(hostname) /etc/hosts | awk '/vtscloud/ { gsub(h".","");print $2}' h=$(hostname)`
...
# set -x
  ...
  L_LOC=${L_TARGET_RUN_FS}/EBSapps/10.1.2/oraInst.loc
  #  update the /etc/oraInst.loc file
  ( cat <<EOF
inventory_loc=${L_LOC}
#inst_group=applmgr
EOF
  ...
  # Update the Oracle HOme oraInst.loc
  ( cat <<EOF
inventory_loc=${L_LOC%%oraInst.loc}oraInventory
#inst_group=applmgr
EOF
) >${L_LOC}
...
function FN_ADCFGCLONE
{
  ...
  if [ ! -d /${L_APPLUSER}/csf/oraInventory ]; then
    mkdir /${L_APPLUSER}/csf/oraInventory
  fi
...
function FN_Change_PWD
{
  ...
  ## Some customers change the password algorithm
  FN_EBS_Profile_Update SIGNON_PASSWORD_HARD_TO_GUESS N SITE
  FN_EBS_Profile_Update SIGNON_PASSWORD_CASE 0 SITE
...
set -x
  ...
  ## EBS_DOMAIN_HOME
  ## en_pwd = encrypt('${L_PWD}','$FMW_HOME/user_projects/domains/EBS_domain_${L_T_SID}')
  ...
  java -cp $L_MW_H/wlserver_10.3/server/lib/weblogic.jar weblogic.WLST <<CATEOF
    ...
    edit()
    startEdit()
    en_pwd = encrypt('${L_PWD}','${EBS_DOMAIN_HOME}')
...
# - - - - - - - - - - - -  Main - - - - - - - - - #
  ...
  if FN_ValidENV $L_T_SID ; then
    FN_Main $L_T_SID 2>&1 | tee -a $DBA/logs/${PG}_${L_S_SID}_${L_T_SID}_$(hostname).$TS.log
  else
