...
# 20151016 pazikasa     INC0975232 - Added Replace to update FND_APEX_URL to SUPP URL
# 20160205 stirlingg    Added year to sysadmin password due to emss password complexity changes                         #
# 20160401 milbyr	INC1301112 - varios changes - search for ticket number						#
# 20180924 milbyr       INC2247443 - change branding to Ladbrokes (FN_EBS_Branding)                                     #
# 20190724 milbyr       added workflow mailer info to context file.							#
# = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = #
#
export FPATH=/export/vtssupp/VTS/fun
#
...
function FN_APPS_Env
{
  case $1 in
    ...
    noappsora | NOAPPSORA )
      ...
      export PATH=/${L_APPLUSER}/apps/tech_st/10.1.3/bin:/usr/ccs/bin:/usr/bin:/usr/vacpp/bin
      ;;
    *)
...
( cd /${L_APPLUSER}/apps/apps_st/comn/clone/bin; /usr/bin/perl adcfgclone.pl appsTier ) |&
...
function FN_EBS_Branding {
  L_PROFILE_DATE=`date +%d-%m-%y`
  L_B_DIR=${DBA}/../FINANCE/GIFS
  L_GIF_FN=LadbrokesCoral_${L_T_SID}_small.gif
  L_GIF=${L_B_DIR}/${L_GIF_FN}
  ...
  FN_EBS_Profile_Update "FND_COLOR_SCHEME" "${L_COLOUR:?TITANIUM}" "SITE"
...
function FN_Change_PWD
{
  ...
  FN_Debug "FNDCPASS apps/$L_APPSPWD 0 Y system/$L_MANAGER USER sysadmin $(FN_PASSWORD_GEN `date +%A` ${L_T_SID}`date +%y` )"
  FNDCPASS apps/$L_APPSPWD 0 Y system/$L_MANAGER USER sysadmin $(FN_PASSWORD_GEN `date +%A` ${L_T_SID}`date +%y` )

  # change password for oracle user and xxlb
  FN_Debug "FNDCPASS apps/$L_APPSPWD 0 Y system/$L_MANAGER ALLORACLE $(FN_PASSWORD_GEN ORACLE ${L_T_SID}`date +%y` )"
  FNDCPASS apps/$L_APPSPWD 0 Y system/$L_MANAGER ALLORACLE $(FN_PASSWORD_GEN ORACLE ${L_T_SID}`date +%y` )

  FN_Debug "FNDCPASS apps/$L_APPSPWD 0 Y system/$L_MANAGER ORACLE XXLB $(FN_PASSWORD_GEN XXLB ${L_T_SID}`date +%y` )"
  FNDCPASS apps/$L_APPSPWD 0 Y system/$L_MANAGER ORACLE XXLB $(FN_PASSWORD_GEN XXLB ${L_T_SID}`date +%y` )
...
### to do - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#      <oa_workflow_server>
#         <hostname oa_var="s_javamailer_imaphost"></hostname>
#         <domain oa_var="s_javamailer_imapdomainname"></domain>
#         <username oa_var="s_wf_admin_role">SYSADMIN</username>
#         <username oa_var="s_javamailer_reply_to"></username>
#         <username oa_var="s_javamailer_imap_user"></username>
#      </oa_workflow_server>
#      <oa_smtp_server>
#         <hostname oa_var="s_smtphost"></hostname>
...
function FN_context_change_new
{
echo $CONTEXT_FILE
  cp $CONTEXT_FILE $CONTEXT_FILE.prechanges

 if [ $L_LCORACLESID = "lbu002" ];
  then
   export URL_ENV='fintest'
 elif [ $L_LCORACLESID = "lbd005" ];
  then
   export URL_ENV='findev'
   elif [ $L_LCORACLESID = "lbsupp" ];
  then
   export URL_ENV='finsupp'
 else
   export URL_ENV='finenv'
 fi


  SMTP_HOST=smtp-relay
  SMTP_DOMAIN=gmail.com:465
  AGENTHOST=`hostname`
  G_DOMAIN=ladbrokescoral.com
  PORTNO=`grep s_https_listen_parameter ${CONTEXT_FILE} | cut -d\> -f2 | cut -d\< -f1`
  URL_PAGE=https://${URL_ENV}.${G_DOMAIN}
  NONSPORT=`grep s_http_listen_parameter ${CONTEXT_FILE} | cut -d\> -f2 | cut -d\< -f1`


  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_smtphost $SMTP_HOST
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_smtpdomainname $SMTP_DOMAIN
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_applcsf /${L_APPLUSER}/csf
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_applptmp /${L_APPLUSER}/csf/temp
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_oacore_nprocs 1
  
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_wfmail_agent http://$L_IASSRV.${G_DOMAIN}:${NONSPORT}

  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_webentryurlprotocol https
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_webentryhost ${URL_ENV}
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_active_webport ${PORTNO}
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_login_page ${URL_PAGE}:${PORTNO}/OA_HTML/AppsLogin
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_external_url ${URL_PAGE}:${PORTNO}

  ## milby 20190724 ##
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_javamailer_outbound_user ${L_WFACCT}
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_javamailer_imap_user     ${L_WFACCT}
  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_javamailer_reply_to      ${L_WFACCT}
  ##  java oracle.apps.ad.context.UpdateContext $CONTEXT_FILE s_test_email group-finance-systems@ladbrokes.co.uk

} ### End function FN_context_change_new
...
function FN_setWorkflow
{
 ...
 export L_DOMAIN="@ladbrokescoral.com"
  ...
  sqlplus -s /nolog <<EOF
    ...
    whenever sqlerror exit FAILURE
      ...
      update fnd_svc_comp_param_vals
      set parameter_value = '${L_WFACCT}@ladbrokescoral.com'
      where parameter_id = '10053';

      prompt Updating Workflow user account...
      update fnd_svc_comp_param_vals
      set parameter_value = 'group-finance-systems@ladbrokes.co.uk'
      ...
      update fnd_svc_comp_param_vals
      set parameter_value = 'smtp-relay.gmail.com:465'
...
--      );
      ...
      update apps.ALR_PROFILE_OPTIONS set PROFILE_OPTION_VALUE='smtp-relay.gmail.com:465' where PROFILE_OPTION_NAME='ORACLE_MAIL_DATABASE';
      select PROFILE_OPTION_NAME, PROFILE_OPTION_VALUE from ALR_PROFILE_OPTIONS where PROFILE_OPTION_NAME='ORACLE_MAIL_DATABASE';
     ...
     DECLARE
       ...
       CURSOR curUsers IS
         ...
         WHERE NVL( end_date, SYSDATE + 1 ) > SYSDATE
           AND user_name NOT IN ('SYSADMIN', 'APPSMGR', 'AUTOINSTALL', 'GUEST','1000496','5272904','5401224','5501190','5501408','7845936');
...
function FN_custom_fndpcesr {
  ...
  for L_CUST_DIR in $XXLGT_TOP $XXLB_TOP 
  do
    echo "= = = = = = = = = $L_CUST_DIR = = = = = = = = ="
    cd ${L_CUST_DIR}/bin && ls -al |grep fndcpesr|awk '{print $9" "$11}'|while read OFN DFN
    do
      ## ls -al $OFN
      DFN=`echo $DFN | sed -e "s/applpd01/${L_APPLUSER}/g"`
      ###mv   $OFN $OFN.orig
      echo " removing $OFN"
      rm  $OFN
...
--    whenever sqlerror exit FAILURE
    set lines 120 pages 200  feedback on echo on  verify on
      ...
      prompt 'FND_APEX_URL profile option value'	 
      select v.proFILE_OPTION_VALUE
      from fnd_profile_option_values V
      where V.PROFILE_OPTION_ID in
        ( select  O.PROFILE_OPTION_ID
          from fnd_profile_options O
          where O.PROFILE_OPTION_NAME = 'FND_APEX_URL')
          and V.PROFILE_OPTION_VALUE  like '%apex.emss.gov.uk%';	 
 
      update fnd_profile_option_values
      set PROFILE_OPTION_VALUE = replace(PROFILE_OPTION_VALUE,'http://apex.emss.gov.uk:8103/apex','http://v06emzs100.edi.emss.gov.uk:8100/apex')
      where PROFILE_OPTION_ID in
        ( select  O.PROFILE_OPTION_ID
          from fnd_profile_options O
          where O.PROFILE_OPTION_NAME = 'FND_APEX_URL')
          and PROFILE_OPTION_VALUE  like '%apex.emss.gov.uk:8103%';	
     commit;
SQLEOF
...
function FN_Perl_fix
{
## perl upgrade fix ##
FN_Debug "FIX: perl 5.10.0 and 5.8.3"
mv /${L_APPLUSER}/apps/tech_st/10.1.3/perl/bin/perl /${L_APPLUSER}/apps/tech_st/10.1.3/perl/bin/perl_old
ln -s /usr/bin/perl /${L_APPLUSER}/apps/tech_st/10.1.3/perl/bin/perl
ln -s /usr/opt/perl5/lib/5.10.1 /${L_APPLUSER}/apps/tech_st/10.1.3/perl/lib/5.8.3
ln -s /usr/opt/perl5/lib/site_perl/5.10.1 /${L_APPLUSER}/apps/tech_st/10.1.3/perl/lib/site_perl/5.8.3
ln -s /usr/opt/perl5/lib/site_perl/5.10.1/aix-thread-multi /${L_APPLUSER}/apps/tech_st/10.1.3/Apache/Apache/mod_perl/lib/site_perl/5.8.3n -s /usr/bin/perl /${L_APPLUSER}/apps/tech_st/10.1.3/perl/bin/perl
...
function FN_Main
{
  ...
  do
	...
	FN_Perl_fix
        ...
        ## milbyr 20180302   ##    investigatye web tier issue ##
...
#        L_COMPLETE=0
      ...
      2)
        ...
        if [ $(hostname) = "${L_CPSRV}" ]; then
          ...
          echo "`date`   milbyr 20180302 - FN_APPS_STOP  $(FN_GetAPPSPassword $L_T_SID)"
        fi
...
#        L_COMPLETE=0
 ...
 #       L_COMPLETE=0
  ...
  #     L_COMPLETE=0
        ;;
      19)
        echo "at step $L_STEP: Apps Stop and Start to resolve weird failure to start"
        if $(FN_IS_CP_TIER)
        then
          echo "This is a web tier and bouncing the services......"
          ## ##echo "Skipping step $L_STEP: Apps Stop and Start to resolve weird failure to start"
          sleep 100
