...
#  parse -e parameter.
#  source vts_utils.sh and vts_vol_utils.sh libraries.
#
################################################################################
{
###
...
###
 ...
 # L_PARAM_FILE=${L_SCRIPT_BASE:-}_params.sh
 ...
 # Source vts_utils.sh file
 if [ -f ${L_SCRIPT_DIR:-}/vts_utils.sh ]
 then
  . ${L_SCRIPT_DIR:-}/vts_utils.sh
  FN_Debug "Sourced the vts_utils functions."
 else
  L_USAGE_ERROR=1
  echo "Cannot find utils file ${L_SCRIPT_BASE:-}/vts_utils.sh"
 fi
...
export L_SCRIPT_BASE=housekeep_files.sh
