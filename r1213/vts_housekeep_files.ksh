...
################################################################################
{
 FN_Debug "FN_Host_details_get:: Start"
  ...
  L_ORAUSER=`echo ora$(echo $G_ENV | tr '[:upper:]' '[:lower:]') | sed 's/_dr$//' | sed 's/mans/man/'  | sed 's/lb//'`
  L_APPLUSER=`echo appl$(echo $G_ENV | tr '[:upper:]' '[:lower:]') | sed 's/_dr$//' | sed 's/mans/man/' | sed 's/lb//'`
